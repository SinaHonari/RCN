...
from RCN.preprocessing.tools import (EOF, padRatio_to_pixels, limit_x,
                                    ...
                                    get_bound_mask)
from RCN.utils.convnet_tools import set_one_for_nonzero
from RCN.utils.bilinear import bilinear_weights
from RCN.preprocessing.local_contrast_normalization import lcn
from RCN.models.layers import (LogisticRegression, ConvPoolLayer, HiddenLayer,
                               PoolLayer, PoolSameSizeLayer, Softmax,
                               UpsamplingLayer)
import os
...
def fforward_model(layer1_input, dropout, nkerns, num_img_channels, dim, rng,
                   use_res_2=False, use_res_1=False, bilinear=False, large_F_filter=False,
                   extra_fine=False, bch_norm=False, concat_pool_locations=False,
                   zero_non_pooled=False, learn_upsampling=False, **kwargs):

    sys.stderr.write("concat_pool_locations is %s.\n" %(concat_pool_locations,))
    sys.stderr.write("zero_non_pooled is %s.\n" %(zero_non_pooled,))
    sys.stderr.write("learn_upsampling is %s.\n" %(learn_upsampling,))
    ...
    # taking the info for the pooled locations
    if concat_pool_locations or zero_non_pooled:
        layerPSS1 = PoolSameSizeLayer(
            input=layerSh1_output,
            patch_size=(2,2)
        )
        layerPSS1_output = layerPSS1.output
    ...
    # taking the info for the pooled locations
    if concat_pool_locations or zero_non_pooled:
        layerPSS2 = PoolSameSizeLayer(
            input=layerSh2_output,
            patch_size=(2,2)
        )
        layerPSS2_output = layerPSS2.output
    ...
    # taking the info for the pooled locations
    if concat_pool_locations or zero_non_pooled:
        layerPSS3 = PoolSameSizeLayer(
            input=layerSh3_output,
            patch_size=(2,2)
        )
        layerPSS3_output = layerPSS3.output
    ...
    # taking the info for the pooled locations
    if concat_pool_locations or zero_non_pooled:
        layerPSS4 = PoolSameSizeLayer(
            input=layerSh4_output,
            patch_size=(2,2)
        )
        layerPSS4_output = layerPSS4.output
    ...
    if use_res_2 or use_res_1:
        ...
        # taking the info for the pooled locations
        if concat_pool_locations or zero_non_pooled:
            layerPSS5 = PoolSameSizeLayer(
                input=layerSh5_output,
                patch_size=(2,2)
            )
            layerPSS5_output = layerPSS5.output
        ...
        if use_res_1:
            ...
            # taking the info for the pooled locations
            if concat_pool_locations or zero_non_pooled:
                layerPSS6 = PoolSameSizeLayer(
                    input=layerSh5_output,
                    patch_size=(2,2)
                )
                layerPSS6_output = layerPSS6.output
            ...
            ratio = 2
            if learn_upsampling:
                layerO3 = UpsamplingLayer(rng=rng, output_grad=layerO2_output,
                                          ratio=ratio)
                layerO3.W.name = 'layerO3_W'
                layerO3_output = layerO3.output
            else:
            ...
            if concat_pool_locations:
                # in this case, in addition to concatenating the branch
                # upsampled values with the pre_pool values, the pool locations
                # is also concatenated by a zero and one tensor
                layerPSS6_zero_one = set_one_for_nonzero(layerPSS6_output)
                layerO4 = T.concatenate((layerSh6_output, layerPSS6_zero_one,
                                         layerO3_output), axis=1)
                layerT1_concat_dim += nkerns[11]
            elif zero_non_pooled:
                # in this case, instead of concatenating with the pre-pooled
                # tensor, a max Relu version of the pre-pooled tensor is used
                # where the pooled value is intact and other values are zeroed.
                layerO4 = T.concatenate((layerPSS6_output, layerO3_output), axis=1)
            else:
                # default case: the concatenation is done with pre-pooled values
        ...
        ratio = 3
        if learn_upsampling:
            layerT3 = UpsamplingLayer(rng=rng, output_grad=layerT2_output,
                                      ratio=ratio)
            layerT3.W.name = 'layerT3_W'
            layerT3_output = layerT3.output
        else:
        ...
        if concat_pool_locations:
            ...
            # upsampled values with the pre_pool values, the pool locations
            # is also concatenated by a zero and one tensor
            layerPSS5_zero_one = set_one_for_nonzero(layerPSS5_output)
            layerT4 = T.concatenate((layerSh5_output, layerPSS5_zero_one,
                                     layerT3_output), axis=1)
            layerS1_concat_dim += nkerns[11]
        elif zero_non_pooled:
            # in this case, instead of concatenating with the pre-pooled
            # tensor, a max Relu version of the pre-pooled tensor is used
            # where the pooled value is intact and other values are zeroed.
            layerT4 = T.concatenate((layerPSS5_output, layerT3_output), axis=1)
        else:
            # default case: the concatenation is done with pre-pooled values
    ...
    ratio = 2
    if learn_upsampling:
        layerS3 = UpsamplingLayer(rng=rng, output_grad=layerS2_output,
                                  ratio=ratio)
        layerS3.W.name = 'layerS3_W'
        layerS3_output = layerS3.output
    elif bilinear:
    ...
    #############################
    layerD1_concat_dim = 0
    if concat_pool_locations:
        print "using concat_pool_locations"
        # in this case, in addition to concatenating the branch
        # upsampled values with the pre_pool values, the pool locations
        # is also concatenated by a zero and one tensor
        layerPSS4_zero_one = set_one_for_nonzero(layerPSS4_output)
        layerS4 = T.concatenate((layerSh4_output, layerPSS4_zero_one,
                                 layerS3_output), axis=1)
        layerD1_concat_dim += nkerns[4]
    elif zero_non_pooled:
        print "using zero_non_pooled"
        # in this case, instead of concatenating with the pre-pooled
        # tensor, a max Relu version of the pre-pooled tensor is used
        # where the pooled value is intact and other values are zeroed.
        layerS4 = T.concatenate((layerPSS4_output, layerS3_output), axis=1)
    else:
        # default case: the concatenation is done with pre-pooled values
    ...
    layerD1 = ConvPoolLayer(
        ...
        bch_norm = bch_norm,
        input=layerS4,
        image_shape=(None, nkerns[13]+nkerns[4]+layerD1_concat_dim, None, None),
        filter_shape=(nkerns[5], nkerns[13]+nkerns[4]+layerD1_concat_dim, conv_large, conv_large),
        act='relu',
        pool=False,
        border_mode='full'
    )
    ...
    ratio = 2
    if learn_upsampling:
        layerD3 = UpsamplingLayer(rng=rng, output_grad=layerD1_output,
                                  ratio=ratio)
        layerD3.W.name = 'layerD3_W'
        layerD3_output = layerD3.output
    elif bilinear:
        ...
        # layerD3_output is now of shape (#batch, #nkerns[5]=5, 20, 20)
    else:
    ...
    #############################
    layerC1_concat_dim = 0
    if concat_pool_locations:
        # in this case, in addition to concatenating the branch
        # upsampled values with the pre_pool values, the pool locations
        # is also concatenated by a zero and one tensor
        layerPSS3_zero_one = set_one_for_nonzero(layerPSS3_output)
        layerD4 = T.concatenate((layerSh3_output, layerPSS3_zero_one,
                                 layerD3_output), axis=1)
        layerC1_concat_dim += nkerns[3]
    elif zero_non_pooled:
        # in this case, instead of concatenating with the pre-pooled
        # tensor, a max Relu version of the pre-pooled tensor is used
        # where the pooled value is intact and other values are zeroed.
        layerD4 = T.concatenate((layerPSS3_output, layerD3_output), axis=1)
    else:
        # default case: the concatenation is done with pre-pooled values
    ...
    layerC1 = ConvPoolLayer(
        ...
        bch_norm = bch_norm,
        input=layerD4,
        image_shape=(None, nkerns[5]+nkerns[3]+layerC1_concat_dim, None, None),
        filter_shape=(nkerns[6], nkerns[5]+nkerns[3]+layerC1_concat_dim, conv_large, conv_large),
        act='relu',
        pool=False,
        border_mode='full'
    )
    ...
    ratio = 2
    if learn_upsampling:
        layerC3 = UpsamplingLayer(rng=rng, output_grad=layerC2_output,
                                  ratio=ratio)
        layerC3.W.name = 'layerC3_W'
        layerC3_output = layerC3.output
    elif bilinear:
        ...
        # layerC3_output is now of shape (#batch, #nkerns[7]=32, 80, 80)
    else:
    ...
    #############################
    layerM1_concat_dim = 0
    if concat_pool_locations:
        # in this case, in addition to concatenating the branch
        # upsampled values with the pre_pool values, the pool locations
        # is also concatenated by a zero and one tensor
        layerPSS2_zero_one = set_one_for_nonzero(layerPSS2_output)
        layerC4 = T.concatenate((layerSh2_output, layerPSS2_zero_one,
                                 layerC3_output), axis=1)
        layerM1_concat_dim += nkerns[2]
    elif zero_non_pooled:
        # in this case, instead of concatenating with the pre-pooled
        # tensor, a max Relu version of the pre-pooled tensor is used
        # where the pooled value is intact and other values are zeroed.
        layerC4 = T.concatenate((layerPSS2_output, layerC3_output), axis=1)
    else:
        # default case: the concatenation is done with pre-pooled values
    ...
    layerM1 = ConvPoolLayer(
        ...
        bch_norm = bch_norm,
        input=layerC4,
        image_shape=(None, nkerns[7]+nkerns[2]+layerM1_concat_dim, None, None),
        filter_shape=(nkerns[8], nkerns[7]+nkerns[2]+layerM1_concat_dim, conv_large, conv_large),
        act='relu',
        pool=False,
        border_mode='full'
    )
    ...
    ratio = 2
    if learn_upsampling:
        layerM3 = UpsamplingLayer(rng=rng, output_grad=layerM2_output,
                                  ratio=ratio)
        layerM3.W.name = 'layerM3_W'
        layerM3_output = layerM3.output
    elif bilinear:
        ...
        # layerM3_output is now of shape (#batch, #nkerns[9]=16, 80, 80)
    else:
    ...
    #############################
    layerF1_concat_dim = 0
    if concat_pool_locations:
        # in this case, in addition to concatenating the branch
        # upsampled values with the pre_pool values, the pool locations
        # is also concatenated by a zero and one tensor
        layerPSS1_zero_one = set_one_for_nonzero(layerPSS1_output)
        layerM4 = T.concatenate((layerSh1_output, layerPSS1_zero_one,
                                 layerM3_output), axis=1)
        layerF1_concat_dim += nkerns[1]
    elif zero_non_pooled:
        # in this case, instead of concatenating with the pre-pooled
        # tensor, a max Relu version of the pre-pooled tensor is used
        # where the pooled value is intact and other values are zeroed.
        layerM4 = T.concatenate((layerPSS1_output, layerM3_output), axis=1)
    else:
        # default case: the concatenation is done with pre-pooled values
    ...
    layerF1 = ConvPoolLayer(
        ...
        bch_norm = bch_norm,
        input=layerM4,
        image_shape=(None, nkerns[9]+nkerns[1]+layerF1_concat_dim, None, None),
        filter_shape=(nkerns[10], nkerns[9]+nkerns[1]+layerF1_concat_dim, conv_large, conv_large),
        act='relu',
        pool=False,
        border_mode='full'
    )
    ...
    if large_F_filter:
        conv_F = 5
        conv_border_F = 2
        sys.stderr.write("using final conv_F of size %s.\n" %(conv_F,))
    else:
        conv_F = conv_small
    # second F layer
    layerF2 = ConvPoolLayer(
        ...
        filter_shape=(nkerns[10], nkerns[10], conv_F, conv_F),
        act='relu',
        pool=False,
        border_mode='full'
    )
    ...
    if conv_F != 1:
        layerF2_output = layerF2.output[:, :, conv_border_F:-conv_border_F, conv_border_F:-conv_border_F]
    else:
    ...
    if learn_upsampling:
        params_branch += layerS3.params + layerD3.params + layerC3.params +\
                         layerM3.params
        if use_res_2 or use_res_1:
            params_branch += layerT3.params
        if use_res_1:
            params_branch += layerO3.params
...
class TCDCN_ConvNet(object):
    def __init__(self, learning_rate, use_ada_delta, decay, train_cost, num_img_channels, nkerns,
                 ...
                 extra_fine=False, large_F_filter=False, only_49_kpts=False, concat_pool_locations=False,
                 zero_non_pooled=False, learn_upsampling=False, **kwargs):

        # indices used for the 49 kpts
        num_kpt_contour = 17
        l_mouth_indx = 61 - 18
        r_mouth_indx = 65 - 19
        ...
        conv_sum, softmax_layer, params, params_no_output, L2_sqr_common, L2_sqr_branch = fforward_model(
            ...
            large_F_filter=large_F_filter, extra_fine=extra_fine,
            concat_pool_locations=concat_pool_locations,
        ...
        if only_49_kpts:
            # excluding the face contour kpts
            kpts_no_contour = error_kpt_each_norm_MTFL_train[:, num_kpt_contour:]
            # excluding the left inner mouth kpt
            kpts_50 = T.concatenate((kpts_no_contour[:, :l_mouth_indx],
                                     kpts_no_contour[:, l_mouth_indx+1:]), axis=1)
            kpts_49 = T.concatenate((kpts_50[:, :r_mouth_indx],
                                     kpts_50[:, r_mouth_indx+1:]), axis=1)
            # error_kpt_each_norm_MTFL is of shape (#batch, #kpts=49))
            error_kpt_each_norm_MTFL_train = kpts_49
        ...
        if only_49_kpts:
            sys.stderr.write(" getting error only for 49 kpts \n")
            # excluding the face contour kpts
            kpts_no_contour = error_kpt_each_norm_MTFL[:, num_kpt_contour:]
            # excluding the left inner mouth kpt
            kpts_50 = T.concatenate((kpts_no_contour[:, :l_mouth_indx],
                                     kpts_no_contour[:, l_mouth_indx+1:]), axis=1)
            kpts_49 = T.concatenate((kpts_50[:, :r_mouth_indx],
                                     kpts_50[:, r_mouth_indx+1:]), axis=1)
            # error_kpt_each_norm_MTFL is of shape (#batch, #kpts=49))
            error_kpt_each_norm_MTFL = kpts_49
...
class Train(object):
    def __init__(self, data_queue, seed_queue, nkerns, num_epochs, learning_rate, batch_size, sliding_window_lenght, task_stop_threshold,
                 L2_coef_common, L2_coef_branch, use_ada_delta, decay, param_path, zero_non_pooled, concat_pool_locations,
                 ...
                 extra_fine, large_F_filter, load_no_output_params, only_49_kpts=False, learn_upsampling=False):
        ...
        tcdcn = TCDCN_ConvNet(learning_rate=learning_rate, use_ada_delta=use_ada_delta, decay=decay, train_cost=train_cost,
                              ...
                              large_F_filter=large_F_filter, only_49_kpts=only_49_kpts,
                              concat_pool_locations=concat_pool_locations,
                              zero_non_pooled=zero_non_pooled,
