...
class TCDCN_ConvNet(object):
    def __init__(self, learning_rate, use_ada_delta, decay, train_cost, num_img_channels, nkerns,
                 ...
                 bilinear=True, bch_norm=False, **kwargs):
        ######################
        # BUILD ACTUAL MODEL #
        ######################
        dim = target_dim
        ...
        layerS1 = ConvPoolLayer(
            ...
            filter_shape=(nkerns[12], nkerns[11], conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        ##################################
        layerDK = T.concatenate((layerD1_output, layerS3_output), axis=1)

        ##########################
        # upsampling layer for D #
        ##########################
        ratio = 2
        layerD3_upsample = T.extra_ops.repeat(layerDK, repeats=ratio, axis=-1)
        ...
        layerC1 = ConvPoolLayer(
            ...
            bch_norm = bch_norm,
            input=layerD4,
            image_shape=(None, nkerns[13]+nkerns[5]+nkerns[3], None, None),
            filter_shape=(nkerns[6], nkerns[13]+nkerns[5]+nkerns[3], conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        ##################################
        layerCK = T.concatenate((layerC2_output, layerD3_output), axis=1)

        ##########################
        # upsampling layer for C #
        ##########################
        ratio = 2
        layerC3_upsample = T.extra_ops.repeat(layerCK, repeats=ratio, axis=-1)
        ...
        layerM1 = ConvPoolLayer(
            ...
            bch_norm = bch_norm,
            input=layerC4,
            image_shape=(None, nkerns[13]+nkerns[5]+nkerns[7]+nkerns[2], None, None),
            filter_shape=(nkerns[8], nkerns[13]+nkerns[5]+nkerns[7]+nkerns[2], conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        ##################################
        layerMK = T.concatenate((layerM2_output, layerC3_output), axis=1)

        ##########################
        # upsampling layer for M #
        ##########################
        ratio = 2
        layerM3_upsample = T.extra_ops.repeat(layerMK, repeats=ratio, axis=-1)
        ...
        layerF1 = ConvPoolLayer(
            ...
            bch_norm = bch_norm,
            input=layerM4,
            image_shape=(None, nkerns[13]+nkerns[5]+nkerns[7]+nkerns[9]+nkerns[1], None, None),
            filter_shape=(nkerns[10], nkerns[13]+nkerns[5]+nkerns[7]+nkerns[9]+nkerns[1], conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        ################################
        # layerF4_3D is of shape (#batch_size * #kpts, #rows, #cols)
        layerF4_3D = layerF4_output.reshape((-1, dim, dim))
        # unormalized_probs is of shape (#batch_size * #kpts, #row * #cols)
        unormalized_probs = layerF4_3D.flatten(2)
        # layerF4_softmax is of shape (#batch_size * #kpts, #row * #cols)
        layerF4_softmax = Softmax(unormalized_probs)
        # layerF4_probs is of shape (#batch_size * #kpts, #row * #cols)
        layerF4_probs = layerF4_softmax.p_y_given_x
        # layerF4_probs becomes of shape (#batch_size, #kpts, #row * #cols)
        layerF4_probs = layerF4_probs.reshape((-1, num_keypoints, dim * dim))
        ...
        L2_sqr_common = ((layerSh1.W ** 2).sum() + (layerSh2.W ** 2).sum() + (layerSh3.W ** 2).sum() + (layerSh4.W ** 2).sum() +\
                         (layerSh5.W ** 2).sum())
        L2_sqr_branch = (layerF1.W ** 2).sum() + (layerF2.W ** 2).sum() + (layerF3.W ** 2).sum() + (layerF4.W ** 2).sum() +\
        ...
        params_branch = layerF1.params + layerF2.params + layerF3.params + layerF4.params + layerM1.params + layerM2.params +\
...
class Train(object):
    def __init__(self, data_queue, seed_queue, nkerns, num_epochs, learning_rate, batch_size, sliding_window_lenght, task_stop_threshold,
                 L2_coef_common, L2_coef_branch, use_ada_delta, decay, param_path, producers, sw_lenght, target_dim, bilinear, bch_norm,
                 train_cost, file_suffix, num_img_channels, sets, param_seed, num_procs, Lambda_coefs, mask_MTFL, mask_300W, use_lcn,
                 num_queue_elem, dropout):
