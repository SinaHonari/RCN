    ...
    def __init__(self, learning_rate, use_ada_delta, decay, train_cost, num_img_channels, nkerns,\
        ...
        if use_res_2 or use_res_1:
            ...
            layerT1 = ConvPoolLayer(
                ...
                bch_norm = bch_norm,
                input = layerT1_input,
                image_shape = (None, nkerns[11]+cumul_concat_dim, None, None),
                filter_shape = (nkerns[11], nkerns[11]+cumul_concat_dim, conv_large, conv_large),
                act='relu',
                pool=False,
                border_mode='full'
            )
            ...
            # learning weights
            ratio = 3
            layerT3_upsample = T.extra_ops.repeat(layerT3_input, repeats=ratio, axis=-1)
        ...
        layerS1 = ConvPoolLayer(
            ...
            input=layerS1_input,
            image_shape=(None, nkerns[11]+cumul_concat_dim, None, None),
            filter_shape=(nkerns[12], nkerns[11]+cumul_concat_dim, conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        ##########################
        ratio = 2
        layerS3_upsample = T.extra_ops.repeat(layerS3_input, repeats=ratio, axis=-1)
        ...
        layerD1 = ConvPoolLayer(
            ...
            bch_norm = bch_norm,
            input=layerS4,
            image_shape=(None, nkerns[4]+cumul_concat_dim, None, None),
            filter_shape=(nkerns[5], nkerns[4]+cumul_concat_dim, conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        ##################################
        layerD3_input = T.concatenate((layerD1_output, layerS3_output), axis=1)

        ##########################
        # upsampling layer for D #
        ##########################
        ratio = 2
        layerD3_upsample = T.extra_ops.repeat(layerD3_input, repeats=ratio, axis=-1)
        ...
        layerC1 = ConvPoolLayer(
            ...
            bch_norm = bch_norm,
            input=layerD4,
            image_shape=(None, nkerns[3]+cumul_concat_dim, None, None),
            filter_shape=(nkerns[6], nkerns[3]+cumul_concat_dim, conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        ##################################
        layerC3_input = T.concatenate((layerC2_output, layerD3_output), axis=1)

        ##########################
        # upsampling layer for C #
        ##########################
        ratio = 2
        layerC3_upsample = T.extra_ops.repeat(layerC3_input, repeats=ratio, axis=-1)
        ...
        layerM1 = ConvPoolLayer(
            ...
            bch_norm = bch_norm,
            input=layerC4,
            image_shape=(None, nkerns[2]+cumul_concat_dim, None, None),
            filter_shape=(nkerns[8], nkerns[2]+cumul_concat_dim, conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        ##################################
        layerM3_input = T.concatenate((layerM2_output, layerC3_output), axis=1)

        ##########################
        # upsampling layer for M #
        ##########################
        ratio = 2
        layerM3_upsample = T.extra_ops.repeat(layerM3_input, repeats=ratio, axis=-1)
        ...
        layerF1 = ConvPoolLayer(
            ...
            bch_norm = bch_norm,
            input=layerM4,
            image_shape=(None, nkerns[1]+cumul_concat_dim, None, None),
            filter_shape=(nkerns[10], nkerns[1]+cumul_concat_dim, conv_large, conv_large),
            act='relu',
            pool=False,
            border_mode='full'
        )
        ...
        for epoch in xrange(num_epochs):
            ...
            for subset in Valid.keys():
                ...
                for index in np.arange(num_batches):
                    ...
                    """
                    cost, cost_kpt, task_cost_vec, error_test, error_kpt, error_kpt_avg, Lambda_vec, L2_cost = tcdcn.valid_model(pose_mask,\
                                     Lambda_coefs, L2_coef_common, L2_coef_branch, x, y_gl, y_gen, y_sm, y_pose, y_kpt_ocular_dist,\
                                     y_kpt_MTFL, y_kpt_300W, mask_MTFL_vec, mask_300W_vec)
                    """
                    cost, cost_kpt, L2_cost, error_kpt, error_kpt_avg = tcdcn.valid_model(L2_coef_common, L2_coef_branch, x,\
                                                                        y_kpt_ocular_dist, y_kpt_MTFL, mask_MTFL_vec, dropout=0)
                    #[cost_gl, cost_gen, cost_sm, cost_pose] = task_cost_vec
