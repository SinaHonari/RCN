...
def tile_images(sample_num, draw_set, out_path, row_size, col_size, model_names):
    rows = 3
    cols = 10
    ...
    total_img = sample_num / float(cols)
    ...
    for kk in xrange(total_img):
        ...
        for model_index, i in enumerate(xrange(0, rows * row_size + gap_rows, row_size + gap_sz)):
            for col_index, jj in enumerate(xrange(0, cols * col_size + gap_cols, col_size + gap_sz)):
                index = kk * cols + col_index + 1
                if index > sample_num:
                    break
                name = "%s_%i_downsampled_%s.png" %(draw_set, index, model_names[model_index])
